...
function avatarUrl(plugin: PluginInstance): string | null {
  const url = avatarCache.value.get(plugin.manifest.name)
  return url || null
}

async function fetchAvatarFor(name: string) {
  if (avatarCache.value.has(name)) return
  try {
    ...
    avatarCache.value.set(name, m ? `https://github.com/${m[1]}.png?size=64` : '')
  } catch {
    avatarCache.value.set(name, '')
  }
}
...
onMounted(async () => {
  ...
  daemon.plugins.forEach((p) => fetchAvatarFor(p.manifest.name))
})
      ...
      >
        <div class="plugin-card-header">
          <div class="plugin-avatar" :class="`plugin-avatar--${plugin.status}`">
            ...
            />
            <NbIcon v-if="isOpenBridge(plugin)" name="diamond" :size="20" />
            <NbIcon v-else name="puzzle-piece" :size="20" />
          </div>
